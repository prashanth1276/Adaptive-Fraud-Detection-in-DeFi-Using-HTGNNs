...
def audit_dataset():
    ...
    edges = pd.read_csv("DataSet/graph_edges.csv", dtype={'src': str, 'dst': str, 'token_address': str}, low_memory=False)
    main = pd.read_csv("DataSet/Ethereum_Fraud_Dataset.csv", dtype={'from_address': str, 'to_address': str})
    ...
    if 'flag' in nodes.columns:
        ...
        print(f"Fraudulent Nodes: {fraud_count} ({ (fraud_count/len(nodes))*100:.2f}%)")
    
    # 4. Check Temporal Alignment
    print(f"\n--- ⏳ Temporal Check ---")
    print(f"Edge Timestamp Range: {edges['timestamp'].min()} to {edges['timestamp'].max()}")
