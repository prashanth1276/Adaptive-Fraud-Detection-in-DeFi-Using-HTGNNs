...
edges_df = edges_df[
    edges_df['src'].isin(node_id_map) &
    edges_df['dst'].isin(node_id_map)
].copy()
...
edges_df = edges_df.sort_values(['edge_type','src_id','dst_id']).reset_index(drop=True)
...
node_last_time = pd.concat(
    [node_last_time_src, node_last_time_dst],
    axis=1
).max(axis=1)
...
g.nodes['node'].data['timestamp'] = torch.tensor(
    nodes_df['timestamp'].values.astype('int64'),
    dtype=torch.long
)
