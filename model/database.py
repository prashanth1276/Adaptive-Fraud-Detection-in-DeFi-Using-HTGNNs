...
class Neo4jKnowledgeGraph:
    def __init__(self, uri, user, password, scaler_path="DataSet/scaler.pkl"):
        ...
        if os.path.exists(scaler_path):
            ...
            self.scaler = None
            print(f"Warning: Scaler not found at {scaler_path}. Features will not be scaled.")

    def close(self):
        self.driver.close()

    def get_subgraph_for_analysis(self, target_address, hops=2, current_time=None):
        with self.driver.session() as session:
            if current_time is None:
                # Fallback to a large number (all edges) – but this breaks temporality.
                current_time = 9999999999

            time_filter = {"relationships": {"timestamp": {"lte": current_time}}}
            query = """
            MATCH (n {address: $addr})
            CALL apoc.path.subgraphAll(n, {
                maxLevel: $hops,
                relationshipFilter: "TRANSACTED>|CALLED>|MINTED>",
                where: $time_filter
            })
            YIELD nodes, relationships
            RETURN nodes, relationships
            """
            result = session.run(query, addr=target_address, hops=hops, time_filter=time_filter).single()
            if not result:
                return None, None
            return self._build_dgl_graph(result['nodes'], result['relationships'], target_address, current_time)
    ...
    def _build_dgl_graph(self, neo4j_nodes, neo4j_rels, target_address, current_time):
        ...
        g.nodes['node'].data['feat'] = torch.tensor(node_feats_scaled, dtype=torch.float32)
    ...
    def get_risk_neighbors(self, address):
        """
        Quick check for your 'Local Network' visualization logic.
        """
        query = """
        MATCH (n {address: $address})-[r]-(m)
        RETURN m.address as neighbor, type(r) as type
        LIMIT 10
        """
        with self.driver.session() as session:
            result = session.run(query, address=address)
            return [f"{row['neighbor'][:10]}... ({row['type']})" for row in result]