    ...
    def forward(self, g):
        ...
        for cetype in g.canonical_etypes:
            if 'timestamp' in g.edges[cetype].data:
                ...
                curr = torch.as_tensor(current_time, device=edge_times.device, dtype=edge_times.dtype)
        ...
        for etype in g.canonical_etypes:
            ...
            g.update_all(dgl.function.copy_e('t_emb', 'm'),
        ...
        if len(temporal_feats) > 0:
            ...
            temporal_feature = torch.zeros((x.shape[0], self.time_encoder.time_dim)).to(x.device)
