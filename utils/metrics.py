...
def compute_metrics(y_true, y_pred, y_prob):
    # Ensure inputs are numpy arrays and handled correctly
    if torch.is_tensor(y_true): y_true = y_true.cpu().numpy()
    if torch.is_tensor(y_pred): y_pred = y_pred.cpu().numpy()
    if torch.is_tensor(y_prob): y_prob = y_prob.cpu().numpy()
    ...
    return {
        ...
        "auprc": average_precision_score(y_true, y_prob) # Critical for fraud research
    }
